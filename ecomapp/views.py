...
class AdminSegmentView(AdminRequiredMixin, TemplateView):
    template_name = "adminpages/segment.html"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        orders = Order.objects.filter(order_status='Order Completed')
        customers = Customer.objects.all()   
        highSeg = SegHighCustomer.objects.all()
        lowSeg = SegLowCustomer.objects.all()
        context = {
            'segcustomers' : customers,
            'orders' : orders,
            'highSeg' : highSeg,
            'lowSeg' : lowSeg 
        }
...
class segmentCustomer(AdminRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        customer = self.kwargs["customer"]
        action = request.GET.get("action")
        customer_obj = Customer.objects.get(id=customer)
        customer_name = customer_obj.full_name
        customer_email = User.objects.get(username=customer_obj.user)
        if action == "segHign":
            SegHighCustomer.objects.create(customer_name = customer_name, customer_email = customer_email.email)
           
        elif action == "segLow":
            SegLowCustomer.objects.create(customer_name = customer_name, customer_email = customer_email.email)
        else:
            pass
        return redirect("ecomapp:adminsegmentview")
...
class AdminOrderListView(AdminRequiredMixin, ListView):
    ...
    queryset = Order.objects.all().order_by("-ordered_by")
